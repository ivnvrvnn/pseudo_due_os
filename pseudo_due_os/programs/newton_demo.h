...
void newtondemo(char** argv, int argc) {
  for(int i = 0; i < 320; i++) {
    ...
    for(int j = 0; j < 240; j++) {     
      ...
      std::complex<float> z((i-160.0)/20.0, (j-120.0)/15.0);
      std::complex<float> zz, zzz;
      std::complex<float> one(1.0, 0.0), two(2.0, 0.0), three(3.0, 0.0);
      int n;
      for(n = 1; n < sizeof(cmap); n++) {        
        zz = z*z; zzz = zz*z;
        c= (zzz-one)/(zz*three);
        z -= c;        
        if (std::norm(c) < 0.000001) break;
      }
